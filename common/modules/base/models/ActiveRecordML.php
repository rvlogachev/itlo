 ...
 * @package common\modules\base\models
 */

use Yii;
use common\modules\base\models\ActiveRecord as ActiveRecordBase;
...
class ActiveRecordML extends ActiveRecordBase
{
     ...
     * @note This variable will be override if you use the `common\modules\yii2-translations` module.
     */
    ...
    public function rules()
    {
        ...
        if ($this->hasAttribute('locale')) {
            ...
            if (isset($this->rulesScenarios['checkLocale'])) {
                ...
                if (isset(Yii::$app->translations) && class_exists('common\modules\translations\models\Languages')) {
                    $rules[] = ['locale', 'exist', 'skipOnError' => true, 'targetClass' => \common\modules\translations\models\Languages::className(), 'targetAttribute' => ['locale' => 'locale']];
                }
            }
        }
    ...
    {
        if (class_exists('\common\modules\translations\models\Languages'))
            return $this->hasOne(\common\modules\translations\models\Languages::class, ['locale' => 'locale']);
        else
            return $this->locale;
    }
        ...
        if (isset($this->alias)) {
            if (isset(Yii::$app->translations) && class_exists('common\modules\translations\models\Languages')) {
                ...
                if ($config = $translations->urlManagerConfig) {
                    ...
                    $urlManager = new \common\modules\translations\components\UrlManager($config);
                } else {
     ...
     * If the `common\modules\yii2-translations` module with the list of active languages is not available,
