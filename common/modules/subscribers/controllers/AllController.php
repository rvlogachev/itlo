...
namespace common\modules\subscribers\controllers;

use Yii;
use common\modules\subscribers\models\Subscribers;
use common\modules\subscribers\models\SubscribersSearch;
use common\modules\subscribers\models\SubscribersImport;
use common\modules\subscribers\models\SubscribersExport;
...
class AllController extends Controller
{
    ...
    public function actionDelete($id)
    {
        if ($this->findModel($id)->delete()) {
            // Log activity
            $this->module->logActivity(
                'Subscriber `'  . '` with ID `' . $id . '` has been successfully deleted.',
                $this->uniqueId . ":" . $this->action->id,
                'success',
                1
            );
            ...
            if (!is_null($import)) {
                if ($model->load(Yii::$app->request->post()) && $model->validate()) {
                    if (($handle = fopen($import->tempName, 'r')) !== false) {
                        ...
                        $subscribers =  ArrayHelper::importCSV($handle, ';', true);
                        fclose($handle);
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->load(Yii::$app->request->post()) && $model->validate()) {
                if ($subscribers = $model->export($model->list_id)) {
                    ...
                    if ($output =  ArrayHelper::exportCSV($subscribers, ['name', 'email'], ";", true)) {
                        // Log activity
                        if (isset($model->list_id)) {
                            $this->module->logActivity(
