...
use common\modules\admin\models\ModulesSearch;
...
//                ]
//            ];
//        } else if ($this->module->moduleExist('admin/rbac')) {
//            // Ok, then we check access according to the rules
//            $behaviors['access'] = [
//                'class' => AccessControl::class,
//                'except' => ['login', 'checkpoint', 'restore'],
//                'rules' => [
...
//        }
    ...
    public function beforeAction($action)
    {
        ...
        if ($module && isset(Yii::$app->params['admin.rememberDuration'])) {
            ...
            Yii::$app->params['users.rememberDuration'] = 1000;
        }
        ...
        if ($module && isset(Yii::$app->params['admin.sessionTimeout'])) {
            ...
            Yii::$app->params['users.sessionTimeout'] = 5000;
        }
    ...
    public function actionModules()
    {
        ...
        if (Yii::$app->user->isGuest) {
            ...
            if (($post = Yii::$app->request->post()) && !Yii::$app->request->isAjax) {
                ...
                if ($action != 'clear') {
                    $module_id = $post['Modules']['extensions'];
                    if (isset($module_id)) {
                        $module_name = explode('/',$module_id )[1] ;
                        ...
                        $composer = BaseFileHelper::normalizePath(Yii::getAlias('@common') .'\modules\\'.$module_name. '\composer.json');
                        ...
                        if (file_exists($composer)) {
                            ...
                            if ($meta = Json::decode($string)) {
                                ...
                                if ($module_id == $meta["name"]) {
                                    ...
                                    $model->setAttribute('class', str_replace('/', '\\', str_replace('backend', 'common\\modules', BaseFileHelper::normalizePath($alias . '\Module'))));
                                    ...
                                    if ($model->validate()) {
                                        ...
                                        Yii::$app->getModule('admin')->setModule($model->module, ArrayHelper::merge([
                                            'class' => $model->class
                                       // ], (is_array($model->options)) ? isset($model->options)?unserialize($model->options):[] : []));
                                        ...
                                        if ($module) {
                                                ...
                                                if ($module->install()) {
                                                    ...
                                                    if($model->save()) {
                                                        ...
                                                        unset($extensions[$model->name]); //TODO unset extensions
           ...
           // return $this->render('index', [
            return $this->render('@common/modules/admin/views/admin/modules/index', [
                'model' => $model,
                'module' => $this->module,
    ...
    public function actionRestore()
    {
        ...
        if ($token) {
            ...
            $model = new \common\modules\users\models\UsersResetPassword($token, [], true);
        ...
        $model = new \common\modules\users\models\UsersPasswordRequest();
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $module->passwordReset = [
                ...
                'emailViewPath' => [
                    'html' => 'common/modules/admin/mail/passwordReset-html',
                    'text' => 'common/modules/admin/mail/passwordReset-text',
                ],
            ];
            ...
            if (Yii::$app->request->isPost && $model->validate()) {
                ...
                if ($model->load(Yii::$app->request->post(), 'DynamicModel')) {
                    ...
                    $message = Yii::$app->mailer
                        ->compose([
                            'html' => 'common/modules/admin/mail/bugReport-html',
                            'text' => 'common/modules/admin/mail/bugReport-text',
                        ], [
                            'name' => $model->name,
                            'email' => $model->email,
    ...
    public function getRecentPages($limit = 5) {
        $model = new \common\modules\pages\models\Pages();
        if (class_exists('\common\modules\users\models\Users')) {
    ...
    public function getRecentPosts($limit = 5) {
        $model = new \common\modules\blog\models\Posts();
        if (class_exists('\common\modules\users\models\Users')) {
            $users = new \common\modules\users\models\Users();
            return $model::find()->select([$model::tableName() . '.id', $model::tableName() . '.name', $model::tableName() . '.created_by', $model::tableName() . '.updated_at'])
                ->joinWith(['createdBy' => function ($query) use ($users) {
    ...
    public function getLastUsers($limit = 5) {
        $model = new \common\modules\users\models\Users();
    ...
    public function getRecentActivity($limit = 5) {
        $model = new \common\modules\activity\models\Activity();
    ...
    public function getRecentComments($limit = 5) {
        $model = new \common\modules\comments\models\Comments();
    ...
    public function getRecentStats() {
        $model = new \common\modules\stats\models\VisitorsSearch();
        ...
        if (isset(Yii::$app->sphinx) && array_key_exists('searchd', $processes)) {
            try {
                if ($sphinx = Yii::$app->sphinx->createCommand('SHOW STATUS;')->queryAll()) {
                    $data['sphinx'] = \common\modules\helpers\src\ArrayHelper::map($sphinx, 'Counter', 'Value');
                }
            } catch (\Exception $e) {
    ...
    private function getNewUsersCount() {
        $counts = \common\modules\users\models\Users::getStatsCount(true, true);
        return [
    ...
    private function getNewOrdersCount() {
        $counts = 0;//\common\modules\store\models\Orders::getStatsCount(true);
        return [
            'count' => ($counts['count']) ? "+" . $counts['count'] : "0",
            'total' => ($counts['total']) ? $counts['total'] : "n/a",
        ];
    ...
    private function getNewCommentsCount() {
        $counts = 0;//\common\modules\comments\models\Comments::find()->count();//getStatsCount(true);

        $counts2 = 0;//\common\modules\reviews\models\Reviews::find()->count();//getStatsCount(true);
        return [
    ...
    private function getNewTransactionsCount() {
        $counts = 0;//\common\modules\billing\models\Transactions::getStatsCount(true);
        return [
            'count' => ($counts['count']) ? "+" . $counts['count'] : "0",
            'total' => ($counts['total']) ? $counts['total'] : "n/a",
        ];
    ...
    private function getNewSubscribersCount() {
        $counts = 0;//\common\modules\subscribers\models\Subscribers::getStatsCount(true);
        return [
            'count' => ($counts['count']) ? "+" . $counts['count'] : "0",
            'total' => ($counts['total']) ? $counts['total'] : "n/a",
        ];
