...
namespace common\modules\mii\generators\model;
...
use yii\db\Schema;
use common\modules\mii\CodeFile;
 ...
 * @since 2.0
 */
class Generator extends \common\modules\mii\Generator
{
    public $db = 'db';
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            if (!$column->allowNull && $column->defaultValue === null) {
                ...
                if($column->dbType=='date' || $column->dbType=='datetime')
                    $defaultsDatetime['default'][] = $column->name;
                elseif($column->phpType=='integer' || $column->phpType=='double' || $column->phpType=='float')
                    $defaultsInt['default'][] = $column->name;
                elseif($column->phpType=='string')
                    $defaultsString['default'][] = $column->name;
            }
            else{
                $defaultsNull['default'][] = $column->name;
            }
            switch ($column->type) {
        ...
        foreach ($types as $type => $columns) {
            if($type=='default')
                $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'value'=>'']";
            else
                $rules[] = "[['" . implode("', '", $columns) . "'], '$type']";
        }
        foreach ($defaultsNull as $type => $columns) {
            $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'value'=>NULL]";
        }
        foreach ($defaultsString as $type => $columns) {
            $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'value'=>'']";
        }
        foreach ($defaultsInt as $type => $columns) {
            $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'value'=>0]";
        }
        foreach ($defaultsDatetime as $type => $columns) {
            $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'value'=>'0000-00-00 00:00:00']";
        }
