...
<?php foreach ($searchRelatedPropertiesModel->properties as $property) : ?>
<?/* foreach ($model->relatedPropertiesModel->toArray($model->relatedPropertiesModel->attributes()) as $name => $value) : */?>
    ...
    <?/* if ($property->filtrable == 'Y') : */?>
        ...
        <?php if ($property->property_type == \skeeks\cms\relatedProperties\PropertyType::CODE_STRING) :?>
            <?= $form->field($searchRelatedPropertiesModel, $name); ?>
        <?php endif; ?>

        <?php if ($property->property_type == \skeeks\cms\relatedProperties\PropertyType::CODE_BOOL) :?>
            <?= $form->field($searchRelatedPropertiesModel, $name)->checkbox(); ?>
        <?php endif; ?>

        <?php if ($property->property_type == \skeeks\cms\relatedProperties\PropertyType::CODE_NUMBER) :?>
