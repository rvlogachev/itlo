...
<div class="col-md-12 sx-dashboard" id="sx-dashboard">
    ...
    <?php if (\Yii::$app->user->can(\common\modules\cms\rbac\CmsManager::PERMISSION_ADMIN_DASHBOARDS_EDIT)) : ?>
    ...
    <div class="row sx-dashboard-body">
        <div class="col-lg-12 col-md-12">
            <?php if (!$dashboard->cmsDashboardWidgets) : ?>
            ...
            <?php else : ?>
                ...
                <table id="sx-dashboard-table">
                    <tr>
                        <?php for($i = 1; $i <= $dashboard->columns; $i++) : ?>
                            <?php $sortableString[] = "#sx-column-" . $i; ?>
                            <td style="width: <?php echo round(100/$dashboard->columns); ?>%;" id="sx-column-<?= $i; ?>" class="sx-columns <?= $i == 1 ? "sx-first": ""?>" data-column="<?= $i; ?>">
                                <?php $widgets = $dashboard->getCmsDashboardWidgets()->andWhere(['cms_dashboard_column' => $i])->orderBy(['priority' => SORT_ASC])->all(); ?>
                                <?php if ($widgets) : ?>

                                    <?php
                                    /**
                                     * @var $widgets \common\modules\cms\models\CmsDashboardWidget[]
                                     */
                                    foreach($widgets as $cmsDashboardWidget) : ?>
                                        ...
                                        <?php
                                            ...
                                            $cmsWidgetData = \yii\helpers\ArrayHelper::merge($cmsWidgetData, [
                                                'editConfigUrl' =>  \common\modules\backend\helpers\BackendUrlHelper::createByParams(['admin/admin-index/edit-dashboard-widget'])
...
HTML;
                                        ...
                                        <?php   \common\modules\cms\modules\backend\widgets\AdminPanelWidget::begin([
                                            ...
                                            <?php if ($cmsDashboardWidget->widget) : ?>
                                                <?php
                                                    try
                                                    {
                                            ...
                                            <?php else : ?>
                                                Виджет удален
                                            <?php endif; ?>
                                        <?php  \common\modules\cms\modules\backend\widgets\AdminPanelWidget::end(); ?>
                            ...
                            <?php if ($dashboard->columns > 1 && $i != $dashboard->columns) : ?>
                                <td width="20">&nbsp;&nbsp;&nbsp;&nbsp;</td>
                            <?php endif; ?>
                        <?php endfor; ?>
                    </tr>
...
CSS
    ...
    $jsonData = \yii\helpers\Json::encode([
        ...
        'backendPrioritySave'       =>  \common\modules\cms\helpers\UrlHelper::construct(['/admin/admin-index/widget-priority-save', 'pk' => $dashboard->id])->enableAdmin()->toString(),
        'backendWidgetRemove'       => \common\modules\cms\helpers\UrlHelper::construct(['/admin/admin-index/widget-remove'])->enableAdmin()->toString(),
    ]);
