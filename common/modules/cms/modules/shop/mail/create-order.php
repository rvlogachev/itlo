...
<div style="text-align: left;">
    ...
    <?php if ($order->shop_pay_system_id) : ?>
        <h4 style="margin-top: 15px;">
        Оплата:
        </h4>
    ...
    <?php if ($order->shopDiscountCoupons) : ?>
        <h4 style="margin-top: 15px;">
        <?= count($order->shopDiscountCoupons) == 1 ? "Скидочный купон:" : "Скидочные купоны:" ;?>
        </h4>


        <?php foreach ($order->shopDiscountCoupons as $shopDiscountCoupon) : ?>
            <b><?= $shopDiscountCoupon->coupon ?></b> -
                <?php if ($shopDiscountCoupon->shopDiscount->value_type == \skeeks\cms\shop\models\ShopDiscount::VALUE_TYPE_F) : ?>
