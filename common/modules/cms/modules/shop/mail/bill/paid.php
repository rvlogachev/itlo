...
<?= Html::beginTag('h1'); ?>
Счет №<?= $model->id; ?> от <?= \Yii::$app->formatter->asDate($model->created_at); ?> по заказу №<?= $model->shopOrder->id ?> успешно оплачен.
<?= Html::endTag('h1'); ?>
...
<div style="text-align: left;">
    ...
    <?= Html::beginTag('p'); ?>
    Сумма: <?= \yii\helpers\Html::tag('b', (string)$model->money); ?>
    <?= Html::endTag('p'); ?>
    ...
    <?= Html::beginTag('p'); ?>
    <?= Html::a("Подробнее о счете", $model->getUrl(true)); ?>
    <?= Html::endTag('p'); ?>
</div>
