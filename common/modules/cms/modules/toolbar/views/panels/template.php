<?php
/* @var $this yii\web\View */
/* @var $panel \common\modules\cms\modules\toolbar\panels\TemplatePanel */
?>

<div class="sx-cms-toolbar__block sx-cms-toolbar__dropdown">
    <nav class="sx-cms-toolbar__label sx-cms-toolbar__dropdown_trigger <?= \Yii::$app->cmsToolbar->editViewFiles == \common\modules\cms\components\Cms::BOOL_Y ? "sx-cms-toolbar__label_success" : ""; ?>">
        <input type="checkbox" value="1"
               onclick="sx.Toolbar.triggerEditViewFiles();" <?= \Yii::$app->cmsToolbar->editViewFiles == \common\modules\cms\components\Cms::BOOL_Y ? "checked" : ""; ?>/>
        <img height="21"
             src="data:image/svg+xml;utf8;base64,"/>


        <span class="sx-cms-toolbar__parent_hover_active">
            <?= \Yii::t('skeeks/toolbar', 'Editing view files', [], \Yii::$app->admin->languageCode) ?>
        </span>
        <?php if ($panel->viewFiles) : ?>
            <?php $files = array_unique($panel->viewFiles); ?>
            <span class="">
                <?= count($files); ?>
            </span>

            <ul class="sx-cms-toolbar__dropdown_menu">
                <?php foreach ($files as $viewFile) : ?>
                    <li><a
                                href="<?= \common\modules\backend\helpers\BackendUrlHelper::createByParams(['/cms/admin-tools/view-file-edit', 'root-file' => $viewFile])->url; ?>"
                                onclick="new sx.classes.toolbar.Dialog('<?= \common\modules\backend\helpers\BackendUrlHelper::createByParams(['/cms/admin-tools/view-file-edit', 'root-file' => $viewFile])->enableEmptyLayout()->url; ?>'); return false;"
                                target="_blank"><?= $viewFile; ?></a></li>
                <?php endforeach; ?>
            </ul>

        <?php endif; ?>

    </nav>
</div>


