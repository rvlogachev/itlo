...
use common\modules\backend\actions\THasActiveForm;
...
class AdminComponentSettingsController extends BackendController
{
    ...
    public function actionCallEdit()
    {
        ...
        if (\Yii::$app->request->get('attributes') && !$settings =  CmsComponentSettings::findByComponent($component)->one()) {
    ...
    public function actionIndex()
    {
        ...
        if ($attibutes && ! CmsComponentSettings::findByComponent($component)->one()) {
    ...
    public function actionRemove()
    {
        ...
        if ($rr->isRequestAjaxPost()) {
            if (\Yii::$app->request->post('do') == 'all') {
                if ($settings =  CmsComponentSettings::findByComponent($component)->all()) {
                    /**
                ...
                if (\Yii::$app->request->post('do') == 'default') {
                    if ($settings =  CmsComponentSettings::findByComponent($component)->one()) {
                        $settings->delete();
                        $component->invalidateCache();
                        $rr->message = 'Настройки успешно удалены';
                        $rr->success = true;
                    ...
                    if (\Yii::$app->request->post('do') == 'sites') {
                        if ($settings =  CmsComponentSettings::findByComponent($component)->andWhere([
                            '>',
                            'cms_site_id',
                            0
                        ])->all()) {
