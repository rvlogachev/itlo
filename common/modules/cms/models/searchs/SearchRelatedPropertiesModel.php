<?php
namespace common\modules\cms\models\searchs;
...
use common\modules\cms\models\CmsContentProperty;
...
class SearchRelatedPropertiesModel extends DynamicModel
{
     ...
     * @var string
     */
    public $propertyElementClassName = '\common\modules\cms\models\CmsContentElementProperty';
    ...
    public function initProperties($props = [])
    {
        foreach ($props as $prop) {
            if ($prop->property_type == \common\modules\cms\relatedProperties\PropertyType::CODE_NUMBER) {
    ...
    public function search(ActiveDataProvider $activeDataProvider, $tableName = 'cms_content_element')
    {
        ...
        foreach ($this->toArray() as $propertyCode => $value) {
            ...
            if ($property = $this->getProperty($propertyCode)) {
                if ($property->property_type == \common\modules\cms\relatedProperties\PropertyType::CODE_NUMBER) {
                    $elementIds = [];
                    ...
                    if ($property->property_type == \common\modules\cms\relatedProperties\PropertyType::CODE_STRING) {
                        ...
                        if ($property->property_type == \common\modules\cms\relatedProperties\PropertyType::CODE_BOOL) {
                            $query = $classSearch::find()->select(['element_id as id'])->where([
                                "value_bool" => $value,
                                "property_id" => $property->id
                            ]);
                            ...
                            if (in_array($property->property_type, [
                                \common\modules\cms\relatedProperties\PropertyType::CODE_ELEMENT
                                ,
                                \common\modules\cms\relatedProperties\PropertyType::CODE_LIST
                                ,
                                \common\modules\cms\relatedProperties\PropertyType::CODE_TREE
                            ])) {
                                $query = $classSearch::find()->select(['element_id as id'])->where([
                                    "value_enum" => $value,
                                    "property_id" => $property->id
