...
namespace common\modules\services\commands;
...
class InitController extends Controller
{
    ...
    public function actionIndex($params = null)
    {
        ...
        if (class_exists('\common\modules\mailer\models\Mails') && $module->moduleLoaded('mailer')) {
            echo "  5) Mailer: Clear cache\n";
        }

        if (class_exists('\common\modules\search\models\Search') && $module->moduleLoaded('search')) {
        ...
        if (class_exists('\common\modules\activity\models\Activity') && $module->moduleLoaded('activity')) {
            echo "  12) Activity: Clear users activity log\n";
        }

        if (class_exists('\common\modules\stats\models\Visitors') && $module->moduleLoaded('stats')) {
            echo "  13) Stats: Clear visitors statistics\n";
        }

        if (class_exists('\common\modules\users\models\Users') && $module->moduleLoaded('users')) {
            echo "  14) Users: Deleting unconfirmed users\n";
            echo "  15) Users: Deleting blocked users\n";
        }

        if (class_exists('\common\modules\api\models\API') && $module->moduleLoaded('api')) {
        ...
        if ($selected == "1") {
            ...
            if (class_exists('\common\modules\activity\models\Activity') && isset(Yii::$app->modules['activity'])) {
            ...
            if (class_exists('\common\modules\stats\models\Visitors') && isset(Yii::$app->modules['stats'])) {
