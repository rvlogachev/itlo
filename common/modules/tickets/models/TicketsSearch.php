...
/**
 * TicketsSearch represents the model behind the search form of `common\modules\tickets\models\Tickets`.
 */
class TicketsSearch extends Tickets
{
    ...
    public function search($params, $current_user = null)
    {
        ...
        if(!is_int($this->task_id) && !empty($this->task_id) && (class_exists('\common\modules\tasks\models\Tasks') && $this->_module->moduleLoaded('tasks'))) {
            $task_id = \common\modules\tasks\models\Tasks::find()->andFilterWhere(['like', 'title', $this->task_id])->one();
