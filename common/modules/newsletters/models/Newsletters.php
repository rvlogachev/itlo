...
use common\modules\validators\src\JsonValidator;
use common\modules\validators\src\EmailsValidator;
...
class Newsletters extends ActiveRecord
{
    ...
    {
        if (class_exists('\common\modules\subscribers\models\Subscribers')) {
            if ($cond) {

                $list = new \common\modules\subscribers\models\Subscribers();
    ...
    public function getRecipients()
    {
        ...
        foreach ($data as $key => $item) {
            if (preg_match('/email_id:(\d+)/', $key)) {
                ...
                if ($list = $this->getSubscribersFromList(['list_id' => intval($match[1]), 'status' => \common\modules\subscribers\models\Subscribers::SUBSCRIBERS_STATUS_ACTIVE], false)) {
    ...
    {
        if (class_exists('\common\modules\subscribers\models\SubscribersList')) {
            if ($cond) {

                $list = new \common\modules\subscribers\models\SubscribersList();
    ...
    public function getCreatedBy()
    {
        if (class_exists('\common\modules\users\models\Users'))
            return $this->hasOne(\common\modules\users\models\Users::class, ['id' => 'created_by']);
        else
    ...
    {
        if (class_exists('\common\modules\users\models\Users'))
            return $this->hasOne(\common\modules\users\models\Users::class, ['id' => 'updated_by']);
        else
