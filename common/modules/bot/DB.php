...
use common\modules\bot\components\telegram\Entities\Chat;
use common\modules\bot\components\telegram\Entities\Message;
use common\modules\bot\components\telegram\Entities\User;
...
class DB
{
     ...
     * @throws Exception
     */
    public static function initialize(array $credentials, Bot $telegram, $table_prefix = null, $encoding = 'utf8mb4')
    {
        if (empty($credentials)) {
            throw new \common\modules\bot\Exception('MySQL credentials not provided!');
        }
        ...
        try {
            ...
            throw new \common\modules\bot\Exception($e->getMessage());
        }
    ...
    public static function externalInitialize($external_pdo_connection, Telegram $telegram, $table_prefix = null)
    {
        if (empty($external_pdo_connection)) {
            throw new \common\modules\bot\Exception('MySQL external connection not provided!');
        }
    ...
    protected static function defineTable()
    {
        if (!defined('TB_TELEGRAM_UPDATE')) {
            define('TB_TELEGRAM_UPDATE', self::$table_prefix.'bot_telegram_update');
        }
        if (!defined('TB_MESSAGE')) {
            define('TB_MESSAGE', self::$table_prefix.'bot_message');
        }
        if (!defined('TB_EDITED_MESSAGE')) {
            define('TB_EDITED_MESSAGE', self::$table_prefix.'bot_edited_message');
        }
        if (!defined('TB_INLINE_QUERY')) {
            define('TB_INLINE_QUERY', self::$table_prefix.'bot_inline_query');
        }
        if (!defined('TB_CHOSEN_INLINE_RESULT')) {
            define('TB_CHOSEN_INLINE_RESULT', self::$table_prefix.'bot_chosen_inline_result');
        }
        if (!defined('TB_CALLBACK_QUERY')) {
            define('TB_CALLBACK_QUERY', self::$table_prefix.'bot_callback_query');
        }
        if (!defined('TB_USER')) {
            define('TB_USER', self::$table_prefix.'bot_user');
        }
        if (!defined('TB_CHAT')) {
            define('TB_CHAT', self::$table_prefix.'bot_chat');
        }
        if (!defined('TB_USER_CHAT')) {
            define('TB_USER_CHAT', self::$table_prefix.'bot_chat_user');
        }
    }
