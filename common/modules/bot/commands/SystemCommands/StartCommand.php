...
//                \Yii::info("Genericmessage fbuser   " . print_r($fbuser, true), 'infostudiobot');
//                Helper::addUser($user_id, 'facebook');
       ...
       // Helper::addUser($this->platform->client,$user_id,$FirstName,$LastName);
        ...
        switch ($this->platform->client) {
            ...
            case 'telegram':
                ...
                $data['text'] = 'Добро пожаловать в Астробот24. Теперь он будет вашим ежедневным "советчиком" в делах. Нажмите "Меню" для продолжения.';
...
//                $data['text'] = 'id= '.$user_id;
            ...
            case 'vk':
                ...
                $txt = 'Добро пожаловать в Астробот24. Теперь он будет вашим ежедневным "советчиком" в делах. Нажмите "Меню" для продолжения.';
                $btn = [
                    'one_time' => true,
                    'buttons' => ['Меню'=>'menu'],
                ];
                ...
                $btnVk = array_merge([],$btn);
                $this->platform = new \common\modules\bot\components\vk\Bot(env('BOT_TOKEN_VKONTAKTE'));
                $result = $this->platform->vkApi_messagesSend($user_id, '-' . $group_id, $txt, json_encode($btnVk, JSON_UNESCAPED_UNICODE));
                $result = Request::emptyResponse();
            ...
            case 'whatsapp':
                ...
                //   \Yii::info("conversation  ---   " . print_r($conversation,true), 'chat');
                ...
                switch ($state) {
                    case 0:
                        ...
                        $conversation->notes['main_step_key'] = $text;
                        $conversation->update();
                        $text = '';
                    ...
                    case 1:
                        if (empty($text)) {
                            ...
                            $cdm = \common\modules\chat\components\Helper::addMessageDbBot($conversation->notes['main_step_key'],(string)$user_id,'in',$this->platform->client,'1' );
                            $txtout = Helper::getScreenWhatsapp($user_id,'whatsapp_start');

                            $cdm = \common\modules\chat\components\Helper::addMessageDbBot($txtout,(string)$user_id,'out',$this->platform->client,'1');
                        }  else {



                            $yesno = Helper::GetAval($text);
                            ...
                            if ($yesno) {
                                ...
                                $conversation->stop();
                                return   $this->platform->executeCommand('startinfo',$yesno[0]);
                                break;
                            } else {
            ...
            case 'facebook':
               ...
               // $botfb->deletePersistentMenu();
                $result = Helper::getScreenFb( $chat_id, 'changelang' ,Helper::GetLanguage($chat_id));
