...
class OrderpartnerCommand extends SystemCommand
{
    /**#@+
     * {@inheritdoc}
     */
    protected $name = 'orderpartner';
    protected $description = 'orderpartner';
...
//            }
        ...
        \Yii::info("orderpartner  text --- ".$text, 'chat');
        //Preparing Respose
        $data = [];
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner1', true);
                            break;
                        } else {
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $result = self::getScreen($user_id,$chat_id, 'partner2', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner3', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner4', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("       Ввод имени  партнера", 'chat');
                            $result = self::getScreen($user_id,$chat_id, 'partner5', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 6:
                        /**
                         * ввод даты рождения партнероа
                         */
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 6;
                            $this->conversation->update();
                            \Yii::info("        ввод даты рождения партнероа", 'chat');
                            $result = self::getScreen($user_id,$chat_id, 'partner6', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 7:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = self::getScreen($user_id,$chat_id, 'partner7', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 8:
                        /**
                         * ввод метса рождения партнера
                         */
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 8;
                            $this->conversation->update();
                            \Yii::info("      ввод метса рождения партнера ", 'chat');
                            $result = self::getScreen($user_id,$chat_id, 'partner8', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 9:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner9', true);
                            break;
                        } else {
                            if (!empty($text) && $text != '❌ Отмена' && $text != '⬅ Назад' && !preg_match('/^[-\w.]+@([а-яёА-ЯЁA-z0-9][-а-яёА-ЯЁA-z0-9]+\.)+[а-яёА-ЯЁA-z]{2,6}$/u', $text)) {
                    ...
                    case 10:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderpartner',$this->conversation->notes);
                            Helper::addOrderAmo('orderpartner',$order,$user_id);
                            ...
                            $inlineButtons[] = [
                                new InlineKeyboardButton(
                                    [
                                        'text' => "Оплатить",
                                        'url' => "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=partner&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']."&platform=telegram&lang=ru-RU&curs=70",
                                    ]
                                )
                            ];
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод имени ", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner1' ,$lang);
                            break;
                        } else {

                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                                $result = Helper::getScreenFb( $chat_id, 'startovyj-ekranfb' ,$lang);
                                break;
                            }
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 2;
                            $this->conversation->update();
                            $result = Helper::getScreenFb( $chat_id, 'partner2' ,$lang);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner1' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner3' ,Helper::GetLanguage($chat_id));
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner2' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод места ", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner4' ,Helper::GetLanguage($chat_id));
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner3' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("       Ввод имени  партнера", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner5' ,Helper::GetLanguage($chat_id));
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner4' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        ввод даты рождения партнероа", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner6' ,Helper::GetLanguage($chat_id));
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner5' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 7:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner7' ,Helper::GetLanguage($chat_id));
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner6' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 8:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("      ввод метса рождения партнера ", 'chat');
                            $result = Helper::getScreenFb( $chat_id, 'partner8' ,Helper::GetLanguage($chat_id));
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner7' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 9:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод эл почта ", 'chat');
                            $txty = Helper::getMessageScreen('partner9' ,'facebook');
                            ...
                            if (!empty($text) && $text != '❌ Отмена' && $text != '⬅ Назад' && !preg_match('/^[-\w.]+@([а-яёА-ЯЁA-z0-9][-а-яёА-ЯЁA-z0-9]+\.)+[а-яёА-ЯЁA-z]{2,6}$/u', $text)) {
                                ...
                                $botfb->send(new Message($chat_id,\Yii::t('frontend',$txt,[],Helper::GetLanguage($chat_id) )));
                                $result = Request::emptyResponse();
                                break;
                            }
                            ...
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner8' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 10:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderpartner',$this->conversation->notes);
                            Helper::addOrderAmo('orderpartner',$order,$user_id,'facebook');
                            \Yii::info("        Оплата ", 'chat');
                            ...
                            $urlbtn =  "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=partner&email=".$this->conversation->notes['vvod_email']."&invoice=".$order->id."&platform=facebook&lang=".$lang."&curs=".$curs;
                            ...
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner9' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод имени ", 'chat');
                            $result = self::getScreenVk( $chat_id, 'partner1-vk' );
                            break;
                        } else {
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 2;
                            $this->conversation->update();
                            $result = self::getScreenVk($user_id,  'partner2-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                ...
                                $result = self::getScreenVk($user_id , 'partner1-vk' );
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = self::getScreenVk($user_id,  'partner3-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод места ", 'chat');
                            $result = self::getScreenVk($user_id , 'partner4-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("       Ввод имени  партнера", 'chat');
                            $result = self::getScreenVk($user_id , 'partner5-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        ввод даты рождения партнероа", 'chat');
                            $result = self::getScreenVk($user_id , 'partner6-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 7:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = self::getScreenVk($user_id, 'partner7-vk', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = self::getScreenVk($user_id,  'partner6-vk', true);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 8:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("      ввод метса рождения партнера ", 'chat');
                            $result = self::getScreenVk($user_id,  'partner8-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = self::getScreenVk($user_id,  'partner7-vk' );
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
                    ...
                    case 9:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreenVk($user_id,  'partner9-vk' );
                            break;
                        } else {
                            if (!empty($text) && $text != '❌ Отмена' && $text != '⬅ Назад' && !preg_match('/^[-\w.]+@([а-яёА-ЯЁA-z0-9][-а-яёА-ЯЁA-z0-9]+\.)+[а-яёА-ЯЁA-z]{2,6}$/u', $text)) {
                                ...
                                $this->platform = new \common\modules\bot\components\vk\Bot(env('BOT_TOKEN_VKONTAKTE'));
                                //"https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=partner&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']
                    ...
                    case 10:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderpartner',$this->conversation->notes);
                             Helper::addOrderAmo('orderpartner',$order,$user_id,'vk');
                            \Yii::info("        Оплата ", 'chat');
                            ...
                            $txt .= "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=partner&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']."&platform=vk&lang=ru-RU&curs=70";
