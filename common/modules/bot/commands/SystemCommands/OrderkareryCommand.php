...
class OrderkareryCommand extends SystemCommand
{
    /**#@+
     * {@inheritdoc}
     */
    protected $name = 'orderkarery';
    protected $description = 'orderkarery';
...
//            }
        ...
        \Yii::info("orderkarery  text --- ".$text, 'chat');
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-karery1', true);
                            break;
                        } else {
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $result = self::getScreen($user_id,$chat_id, 'partner-karery2', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-karery3', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-karery4', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-karery5', true);
                            break;
                        } else {
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderkarery',$this->conversation->notes);
                            Helper::addOrderAmo('orderkarery',$order,$user_id);
                            ...
                            $inlineButtons[] = [
                                new InlineKeyboardButton(
                                    [
                                        'text' => "Оплатить",
                                        'url' => "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=karery&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']."&platform=telegram&lang=ru-RU&curs=70",
                                    ]
                                )
                            ];
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод имени ", 'chat');
                            $result = self::getScreenVk($user_id,  'partner-karery1-vk' );
                            break;
                        } else {
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 2;
                            $this->conversation->update();
                            $result = self::getScreenVk($user_id,  'partner-karery2-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = self::getScreenVk($user_id,  'partner-karery3-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод места ", 'chat');
                            $result = self::getScreenVk($user_id,  'partner-karery4-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreenVk($user_id,  'partner-karery5-vk' );
                            break;
                        } else {
                            if (!empty($text) && $text != '❌ Отмена' && $text != '⬅ Назад' && !preg_match('/^[-\w.]+@([а-яёА-ЯЁA-z0-9][-а-яёА-ЯЁA-z0-9]+\.)+[а-яёА-ЯЁA-z]{2,6}$/u', $text)) {
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderkarery',$this->conversation->notes);
                            Helper::addOrderAmo('orderkarery',$order,$user_id,'vk');
                            \Yii::info("        Оплата ", 'chat');
                            $txt = \common\modules\widget\widgets\DbText::widget(['key' => 'txt_for_pay']);
                            $txt .= "\r\n\r\n";
                            $txt .= "Для оплаты пройдите по ссылке:\r\n";
                            $txt .= "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=karery&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']."&platform=vk&lang=ru-RU&curs=70";
                            $this->platform = new \common\modules\bot\components\vk\Bot(env('BOT_TOKEN_VKONTAKTE'));
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод имени ", 'chat');
                            $result = Helper::getScreenFb( $user_id, 'partner-karery1' ,$lang);
                            break;
                        } else {
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 2;
                            $this->conversation->update();
                            $result = Helper::getScreenFb( $user_id, 'partner-karery2' ,$lang);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = Helper::getScreenFb( $user_id, 'partner-karery3' ,$lang);
                            break;
                        }else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод места ", 'chat');
                            $result = Helper::getScreenFb( $user_id, 'partner-karery4' ,$lang);
                            break;
                        }else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод эл почта ", 'chat');
                            $txty = Helper::getMessageScreen('partner-karery5' ,'facebook');
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderkarery',$this->conversation->notes,'facebook');
                            Helper::addOrderAmo('orderkarery',$order,$user_id,'facebook');
                            \Yii::info("        Оплата ", 'chat');
                            ...
                            $urlbtn =  "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=karery&email=".$this->conversation->notes['vvod_email']."&invoice=".$order->id."&platform=facebook&lang=".$lang."&curs=".$curs;
                            ...
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb( $user_id, 'partner-karery5' ,$lang);
                                break;
