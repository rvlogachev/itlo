...
class OrderzdorovaCommand extends SystemCommand
{
    /**#@+
     * {@inheritdoc}
     */
    protected $name = 'orderzdorova';
    protected $description = 'orderzdorova';
...
//            }
        ...
        \Yii::info("orderzdorova  text --- ".$text, 'chat');
        //Preparing Respose
        $data = [];
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-zdorova1', true);
                            break;
                        } else {
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $result = self::getScreen($user_id,$chat_id, 'partner-zdorova2', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-zdorova3', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-zdorova4', true);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreen($user_id,$chat_id, 'partner-zdorova5', true);
                            break;
                        } else {
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderzdorova',$this->conversation->notes);
                            Helper::addOrderAmo('orderzdorova',$order,$user_id);
                            ...
                            $inlineButtons[] = [
                                new InlineKeyboardButton(
                                    [
                                        'text' => "Оплатить",
                                        'url' => "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=zdorova&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']."&platform=telegram&lang=ru-RU&curs=70",
                                    ]
                                )
                            ];
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод имени ", 'chat');
                            $result = self::getScreenVk($user_id, 'partner-zdorova1-vk');
                            break;
                        } else {
                            ...
                            if ($text == '⬅ Назад') {
                                ...
                                $result = self::getScreenVk($user_id,  'individualnye-goroskopy-vk' );
                                break;
                            }
                        }
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 2;
                            $this->conversation->update();
                            $result = self::getScreenVk($user_id,  'partner-zdorova2-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = self::getScreenVk($user_id, 'partner-zdorova3-vk' );
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод места ", 'chat');
                            $result = self::getScreenVk($user_id, 'partner-zdorova4-vk');
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $result = self::getScreenVk($user_id,  'partner-zdorova5-vk' );
                            break;
                        } else {
                            if (!empty($text) && $text != '❌ Отмена' && $text != '⬅ Назад' && !preg_match('/^[-\w.]+@([а-яёА-ЯЁA-z0-9][-а-яёА-ЯЁA-z0-9]+\.)+[а-яёА-ЯЁA-z]{2,6}$/u', $text)) {
                            ...
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state - 1;
                                $this->conversation->update();
                                $result = self::getScreenVk($user_id,  'partner-zdorova4' );
                                break;
                            }
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            $order = Helper::addOrder('orderzdorova',$this->conversation->notes);
                            Helper::addOrderAmo('orderzdorova',$order,$user_id,'vk');
                            \Yii::info("        Оплата ", 'chat');
                            $txt = \common\modules\widget\widgets\DbText::widget(['key' => 'txt_for_pay']);
                            $txt .= "\r\n\r\n";
                            $txt .= "Для оплаты пройдите по ссылке:\r\n";
                            $txt .= "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=zdorova&invoice=".$order->id."&email=".$this->conversation->notes['vvod_email']."&platform=vk&lang=ru-RU&curs=70";
                            $this->platform = new \common\modules\bot\components\vk\Bot(env('BOT_TOKEN_VKONTAKTE'));
                ...
                switch ($state) {
                    ...
                    case 1:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод имени ", 'chat');
                            $result = Helper::getScreenFb( $user_id, 'partner-zdorova1' ,$lang);
                            break;
                    ...
                    case 2:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 2;
                            $this->conversation->update();
                            $result = Helper::getScreenFb( $user_id, 'partner-zdorova2' ,$lang);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 3:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Выбор модели ", 'chat');
                            $result = Helper::getScreenFb( $user_id, 'partner-zdorova3' ,$lang);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 4:
                        ...
                        if (empty($text)) {
                            ...
                            $this->conversation->update();
                            \Yii::info("        Ввод места ", 'chat');
                            $result = Helper::getScreenFb( $user_id, 'partner-zdorova4' ,$lang);
                            break;
                        } else {
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                    ...
                    case 5:
                        ...
                        if (empty($text)) {
                            ...
                            $txty = Helper::getMessageScreen('partner-zdorova5' ,'facebook');
                    ...
                    case 6:
                        ...
                        if (empty($text)) {
                            $this->conversation->notes['state'] = 6;
                            $this->conversation->update();
                            $order = Helper::addOrder('orderzdorova',$this->conversation->notes,'facebook');
                            Helper::addOrderAmo('orderzdorova',$order,$user_id,'facebook');
                            \Yii::info("        Оплата ", 'chat');
                            ...
                            $urlbtn =  "https://astrobot24.online/chat/default/pay/?chatid=".$chat_id."&tarif=zdorova&email=".$this->conversation->notes['vvod_email']."&invoice=".$order->id."&platform=facebook&lang=".$lang."&curs=".$curs;
                            ...
                            if ($text == '⬅ Назад') {
                                $this->conversation->notes['state'] = $state-1;
                                $this->conversation->update();
                                $result = Helper::getScreenFb($user_id , 'partner-zdorova5' ,$lang);
                                break;
                            }
                            if ($text == '❌ Отмена') {
                                $this->conversation->stop();
