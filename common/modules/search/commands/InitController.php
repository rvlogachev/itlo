...
namespace common\modules\search\commands;

use common\modules\search\models\LiveSearch;
use common\modules\search\models\Search;
use common\modules\search\models\SearchIndex;
use common\modules\search\models\SearchKeywords;
use Yii;
...
class InitController extends Controller
{
    ...
    public function actionIndex($params = null)
    {
        ...
        if ($selected == "1") {
            Yii::$app->runAction('migrate/up', ['migrationPath' => '@common/modules/search/migrations', 'interactive' => true]);
        } else if ($selected == "2") {
            Yii::$app->runAction('migrate/down', ['migrationPath' => Yii::getAlias('@common').'/modules/search/migrations', 'interactive' => true]);
        } else if ($selected == "3") {
            ...
            if ($urlManager = Yii::$app->getUrlManager()) {
                ...
                if (is_null($urlManager->getHostInfo()))
                    echo $this->ansiFormat("\nError! `hostInfo` must be configured in the UrlManager component.\n", Console::FG_RED);

                if (is_null($urlManager->getBaseUrl()))
